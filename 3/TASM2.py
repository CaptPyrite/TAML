...
def sub_handler(args):
    ...
    mov_handler([int(num1) - int(num2), dest])

@command("mul")
def mul_handler(args):
    num1, num2, dest = args[0], args[1], args[2]
    num1 = resolve(num1)
    num2 = resolve(num2)
...
def push_handler(args):
    ...
    RAM[dest] = src # Saves from registry to RAM

@command("comp")
def comp_handler(args):
    reg1, reg2, dest = args[0], args[1], args[2]
    reg1 = resolve(reg1)
    reg2 = resolve(reg2)
    value = reg1 == reg2
